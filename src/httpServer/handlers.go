...
func (s *Server) GetKeyDownButtonHandler() appHandler {
	return func(w http.ResponseWriter, r *http.Request) *appError {
		client, id, errorInfo := s.getClient(r)
		if errorInfo != nil {
			return errorInfo
		...
		if len([]rune(t.Button)) != 0 {
			result, err := client.KeyDown(t.Button)
			if err != nil || result == false {
				status := responseStatuses["UnknownError"]
				return &appError{err.Error(), http.StatusInternalServerError, &status}
			}
		} else {
			return &appError{"button is required.", http.StatusBadRequest, nil}
		}
		response := &SessionResponse{
			Status: responseStatuses["Success"],
			Id:     id,
...
func (s *Server) GetKeyUpButtonHandler() appHandler {
	return func(w http.ResponseWriter, r *http.Request) *appError {
		client, id, errorInfo := s.getClient(r)
		if errorInfo != nil {
			return errorInfo
		...
		if len([]rune(t.Button)) != 0 {
			result, err := client.KeyUp(t.Button)
			if err != nil || result == false {
				status := responseStatuses["UnknownError"]
				return &appError{err.Error(), http.StatusInternalServerError, &status}
			}
